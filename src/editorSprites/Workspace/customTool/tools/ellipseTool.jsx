...
const EllipseTool = ({ setToolParameters, toolConfigs, setToolConfigs }) => {
  const [borderWidth, setBorderWidth] = useState(
    () => toolConfigs?.ellipse?.borderWidth ?? DEFAULT,
  );

  useEffect(() => {
    setToolConfigs((prev) => ({ ...prev, ellipse: { borderWidth } }));
    ...
    <div className="polygon-tool-container">
      <div className="tool-configs">
        <div className="config-item">
          <label className="tool-label" htmlFor="ellipseTool-borderWidth">
            Border Width
          </label>
          <div className="input-container">
            <input
              id="ellipseTool-borderWidth"
              type="number"
              min={MIN}
              max={MAX}
              value={borderWidth}
