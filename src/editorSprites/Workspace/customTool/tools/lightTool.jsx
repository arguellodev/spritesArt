...
const LightTool = ({ setToolParameters, tool, toolParameters, toolConfigs, setToolConfigs }) => {
  ...
  const [intensity, setIntensity] = useState(0.5); // Control de intensidad del iluminado
  ...
  useEffect(() => {
    ...
    if (lightConfig !== null) {
      ...
      setVertices(lightConfig.vertices || 5);
      setRotation(lightConfig.rotation || 0);
      setPattern(lightConfig.pattern || "solid");
      setPressure(lightConfig.pressure || 50);
      setSharpen(lightConfig.sharpen || 0);
      setPaintMode(lightConfig.paintMode || 'manual');
      setVelocitySensibility(lightConfig.velocitySensibility || 0);
      setPatternAlignment(lightConfig.patternAlignment || 'normal');
      setIntensity(lightConfig.intensity || 0.5);
      setSelectedBrushType(lightConfig.selectedBrushType || 'estandar');
    }
  ...
  useEffect(() => {
    if (typeof borderWidth === 'number' && 
      ...
      setToolParameters(prev => ({
        ...
        intensity: intensity, // Control de intensidad del iluminado
